...
import {
  igamingSqlExerciseGroups,
  getIgamingSqlExercise,
  type SqlExercise,
} from "@/content/igaming-sql-exercises";
import { progressStore, useProgress } from "@/lib/progress";
import { IgamingTableExplorer } from "./igaming-table-explorer";
import { SqlResults } from "./sql-results";

const IGAMING_SOURCE = { getTable: getIgamingTable, tableNames: igamingTableNames };

const DEFAULT_QUERY = `-- The iGaming SQL Lab runs the same in-browser SQL engine as the SQL for
-- Testers module, over a separate 13-table synthetic gambling-platform dataset.

SELECT p.username, w.balance, w.bonus_balance
FROM players p
...
export function IgamingSqlLab() {
  ...
  function run(sql = query) {
    try {
      const outcome = executeSql(sql, IGAMING_SOURCE);
      setResult(outcome);
      setError(null);
      return outcome;
    } catch (thrown) {
  ...
  function checkAnswer() {
    ...
    try {
      expected = executeSql(exercise.solution, IGAMING_SOURCE);
    } catch {
  ...
  function finish(passed: boolean, message: string) {
    if (passed && exercise) progressStore.completeIgamingSql(exercise.id);
  ...
  return (
    <div className="flex min-h-[75vh] flex-col lg:flex-row">
      ...
      <aside className="scrollbar-thin w-full shrink-0 overflow-y-auto border-b border-line bg-surface p-3 lg:w-72 lg:border-b-0 lg:border-r">
        <IgamingTableExplorer onInsert={insert} />
        ...
        <div className="mt-6">
          ...
          {igamingSqlExerciseGroups.map((group) => (
            <div key={group.group} className="mb-3">
              <p className="mb-1 px-1 text-[11px] text-faint">{group.group}</p>
              <ul className="space-y-0.5">
                {group.items.map((item) => {
                  const done = Boolean(progress.igamingSql[item.id]);
                  return (
                    <li key={item.id}>
                      <button
                        type="button"
        ...
        {exercise && (
          <div className="border-b border-line bg-surface-2 px-4 py-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {progress.igamingSql[exercise.id] && <Badge tone="accent">Solved</Badge>}
            </div>
