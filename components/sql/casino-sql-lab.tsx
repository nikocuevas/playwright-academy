...
import {
  casinoSqlExerciseGroups,
  getCasinoSqlExercise,
  type SqlExercise,
} from "@/content/casino-sql-exercises";
import { progressStore, useProgress } from "@/lib/progress";
import { CasinoTableExplorer } from "./casino-table-explorer";
import { SqlResults } from "./sql-results";

const CASINO_SOURCE = { getTable: getCasinoTable, tableNames: casinoTableNames };

const DEFAULT_QUERY = `-- The Casino SQL Lab runs the same in-browser SQL engine as the SQL for
-- Testers module, over a separate four-table European Roulette dataset.

SELECT p.username, r.result, b.bet_type, b.status, b.payout
FROM players p
...
export function CasinoSqlLab() {
  ...
  function run(sql = query) {
    try {
      const outcome = executeSql(sql, CASINO_SOURCE);
      setResult(outcome);
      setError(null);
      return outcome;
    } catch (thrown) {
  ...
  function checkAnswer() {
    ...
    try {
      expected = executeSql(exercise.solution, CASINO_SOURCE);
    } catch {
  ...
  function finish(passed: boolean, message: string) {
    if (passed && exercise) progressStore.completeCasinoSql(exercise.id);
  ...
  return (
    <div className="flex min-h-[75vh] flex-col lg:flex-row">
      ...
      <aside className="scrollbar-thin w-full shrink-0 overflow-y-auto border-b border-line bg-surface p-3 lg:w-72 lg:border-b-0 lg:border-r">
        <CasinoTableExplorer onInsert={insert} />
        ...
        <div className="mt-6">
          ...
          {casinoSqlExerciseGroups.map((group) => (
            <div key={group.group} className="mb-3">
              <p className="mb-1 px-1 text-[11px] text-faint">{group.group}</p>
              <ul className="space-y-0.5">
                {group.items.map((item) => {
                  const done = Boolean(progress.casinoSql[item.id]);
                  return (
                    <li key={item.id}>
                      <button
                        type="button"
        ...
        {exercise && (
          <div className="border-b border-line bg-surface-2 px-4 py-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {progress.casinoSql[exercise.id] && <Badge tone="accent">Solved</Badge>}
            </div>
