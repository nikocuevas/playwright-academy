...
import { useIgaming } from "@/components/igaming/igaming-provider";

function LoginForm() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { refreshSession, user } = useIgaming();
  ...
  const next = searchParams.get("next") ?? "/practice/igaming";
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/igaming/auth/login", {
        method: "POST",
  ...
  return (
    <div className="mx-auto max-w-md">
      ...
      >
        <h1 className="text-xl font-semibold tracking-tight">Sign in to FakeBet Casino</h1>
        <p className="mt-1 text-sm text-muted">
          Wallet, betting and responsible gambling tools require an account.
        </p>
        ...
        <div className="mt-5 rounded-lg border border-line bg-surface-2 p-3 text-xs">
          ...
          <p className="mt-1 font-mono text-muted">player@example.com</p>
          <p className="font-mono text-muted">Password123!</p>
          <p className="mt-2 text-faint">
            Entirely fictional. Fake balances, fake bets — never enter real details into a
            practice app.
          </p>
        </div>
      </form>
...
export default function IgamingLoginPage() {
  return (
    <React.Suspense
